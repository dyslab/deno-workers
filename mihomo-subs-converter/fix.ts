...
export async function getHandler(request: Request): Promise<Response> {
  ...
  if (link) {
    try {
      ...
      // 1. Remove invalid html tags '<pre>' & '</pre>'.
      processingText = processingText.replace(/\<\/?pre.*\>/gm, '');
      console.log(`1. The process of removing invalid html tags '<pre>' & '</pre>' accomplished.`);
      // 2. Remove invalid yaml string '!&lt;str&gt; '.
      processingText = processingText.replace(/!&lt;(str)?&gt;\s/gm, '');
      console.log(`2. The process of removing invalid yaml string '!&lt;str&gt; ' accomplished.`);
      // 3. Remove SS nodes with the incorrect configuration of 'cipher: ss'.
      ...
      if (yamlObj['proxies']) {
        ...
        console.log(`3. The process of removing SS nodes with the incorrect configuration of 'cipher: ss' accomplished. Counted proxies: ${oldProxiesCount} => ${yamlObj['proxies'].length}, total ${oldProxiesCount - yamlObj['proxies'].length} proxies removed.`);
